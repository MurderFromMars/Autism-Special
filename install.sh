...
REPO_DIR="$HOME/Autism-Special"
BACKUP_DIR="$HOME/Autism-Special-backup-$(date +%Y%m%d_%H%M%S)"
DISTRO="unknown"
WALLPAPER_PATH="/usr/local/share/icons/autismwallpaper.png"
...
fetch_repo() {
    log "syncing Autism-Special repository…"
    if [ ! -d "$REPO_DIR/.git" ]; then
        if git clone https://github.com/MurderFromMars/Autism-Special "$REPO_DIR" 2>&1 | grep -v -E "^(remote:|Receiving|Resolving|Counting)" | grep -v "^$" || false; then
...
install_arch_dependencies() {
    ...
    sudo pacman -S --needed --noconfirm \
        ...
        kitty vulkan-headers fastfetch imagemagick ttf-jetbrains-mono >/dev/null 2>&1
...
deploy_kora_icons() {
    ...
    local kora_archive="$REPO_DIR/kora-2-0-4.tar.xz"
    local kora_dest="$HOME/.local/share/icons"
    if [ -f "$kora_archive" ]; then
        [ -d "$kora_dest/kora" ] && rm -rf "$kora_dest/kora"
        tar xf "$kora_archive" -C "$kora_dest"
        ok "icons → kora"
    else
        warn "kora-2-0-4.tar.xz not found at $kora_archive"
    fi
}
...
deploy_sounds() {
    log "deploying sounds…"
    local src="$REPO_DIR/sounds"
    local dest="$HOME/.local/share/sounds"
    if [ -d "$src" ]; then
        mkdir -p "$dest"
        cp -r "$src/." "$dest/"
        ok "sounds → ~/.local/share/sounds"
    else
        warn "missing → sounds directory"
    fi
}

deploy_color_scheme() {
    log "installing AutismSpecial color scheme…"
    mkdir -p "$HOME/.local/share/color-schemes"
    if [ -f "$REPO_DIR/AutismSpecial.colors" ]; then
        cp "$REPO_DIR/AutismSpecial.colors" "$HOME/.local/share/color-schemes/"
        ok "colors → AutismSpecial"
    else
        warn "AutismSpecial.colors missing"
    fi
}
...
deploy_wallpapers() {
    ...
    if [ -f "$REPO_DIR/autismwallpaper.png" ]; then
        sudo cp "$REPO_DIR/autismwallpaper.png" /usr/local/share/icons/
        ok "wallpaper → autismwallpaper.png → /usr/local/share/icons"
    else
        warn "missing → autismwallpaper.png"
    fi

    if [ -f "$REPO_DIR/blackarch.png" ]; then
        sudo cp "$REPO_DIR/blackarch.png" /usr/local/share/icons/
        ok "icon → blackarch.png → /usr/local/share/icons"
    else
        warn "missing → blackarch.png"
    fi

    mkdir -p "$HOME/Pictures"
    if [ -f "$REPO_DIR/autismarch.png" ]; then
        cp "$REPO_DIR/autismarch.png" "$HOME/Pictures/"
        ok "wallpaper → autismarch.png → ~/Pictures"
    else
        warn "missing → autismarch.png"
    fi
}
...
apply_kde_theme_settings() {
    ...
    if command -v plasma-apply-colorscheme >/dev/null 2>&1; then
        plasma-apply-colorscheme AutismSpecial 2>/dev/null || true
        ok "color scheme activated → AutismSpecial"
    else
...
main() {
    printf "\n\033[1;35m┌───────────────────────────────────────────────────────┐\n"
    printf   "│   AUTISM-SPECIAL DYNAMIC TILING THEME BY MURDERFROMMARS  │\n"
    printf   "└───────────────────────────────────────────────────────┘\033[0m\n\n"
    ...
    printf "\n\033[1;35m╔═══════════════════════════════════════════════════════╗\033[0m\n"
    printf "\033[1;35m║\033[0m  \033[1;32mAUTISM-SPECIAL DEPLOYMENT COMPLETE\033[0m                \033[1;35m║\033[0m\n"
    printf "\033[1;35m╚═══════════════════════════════════════════════════════╝\033[0m\n\n"
